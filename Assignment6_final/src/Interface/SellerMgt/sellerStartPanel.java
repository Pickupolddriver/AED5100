...
//        setOrder();
        populate();
        txtBalance.setText(String.valueOf(seller.getBank().getBalance()));
...
//    }
    ...
    public void populateOrder() {
        ...
        dtm.setRowCount(0);
        //直接populate business里面的---似乎没有加上if判断语句
        for (WorkRequest o : business.getWq().getWorkRequestList()) {
            if(o.getOrder().getSeller().equals(seller)){
            Object row[] = new Object[7];
            row[0] = o;
            row[1] = o.getOrder().getOrderDate();
            row[2] = o.getOrder().getWine();
            row[3] = o.getOrder().getNewBuyer();
            row[4] = o.getOrder().getQty();
            row[5] = o.getOrder().getTotal();
            row[6] = o.getOrder().getOrderStatus();
            //row[7] = o.getStatus();
            System.out.print(o.getOrder().getOrderStatus());
            dtm.addRow(row);}
        }
    }
    ...
    private void initComponents() {
        ...
        btnReturn.setText("Comfirm a Messege");
        ...
        btnReturn1.setFont(new java.awt.Font("Viner Hand ITC", 1, 18)); // NOI18N
        btnReturn1.setForeground(new java.awt.Color(255, 102, 102));
        btnReturn1.setText("Comfirm a Refund");
    ...
    private void btnUpdateShippingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateShippingActionPerformed
        ...
        WorkRequest ww = (WorkRequest) tblTransaction.getValueAt(rowSelect, 0);

        if (rowSelect >= 0) {

            System.out.print(ww.getStatus());
            updateShippingInfoPanel usip = new updateShippingInfoPanel(processPanel, ww);
...
//    }     
        ...
        WorkRequest oo = (WorkRequest) tblTransaction.getValueAt(rowSelect, 0);
        if (rowSelect >= 0) {
...
//            }
            ...
            String c = oo.getOrder().getOrderStatus();
            if (c.equals("Return started")) {
                oo.getOrder().setOrderStatus("Returned");
                oo.setStatus("Pending");
           
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                String str = sdf.format(new Date());
               oo.setRequestDate(str);
                populateOrder();
                for (Wine www : seller.getWineList()) {
                    if (www.getWineName().equals(oo.getOrder().getWine().getWineName())) {
                        int a = www.getNumberofInventory() + oo.getOrder().getQty();
                        www.setNumberofInventory(a);
                    }

                }
                JOptionPane.showMessageDialog(null, "Well done");
            } else if (c.equals("Order Paid")) {
                oo.getOrder().setOrderStatus("Shipping");
                oo.setStatus(c);
                populateOrder();
            ...
            else if (c.equals("Shipping")) {
                oo.getOrder().setOrderStatus("Shipped");
                oo.setStatus(c);
                populateOrder();
                //setWineNumber(oo.getQty(),oo.getWine().getWineName());
                JOptionPane.showMessageDialog(null, "Well done");
...
// TODO add your handling code here:
    ...
    private void btnAppealActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAppealActionPerformed
        ...
        WorkRequest oo = (WorkRequest) tblTransaction.getValueAt(rowSelect, 0);
        if (rowSelect >= 0) {
            oo.getOrder().setOrderStatus("Appealed");
            oo.setStatus("Appeal processing");
            JOptionPane.showMessageDialog(null, "Appealing");
        } else {
            JOptionPane.showMessageDialog(null, "Please select any row");
        }
        populateOrder();
