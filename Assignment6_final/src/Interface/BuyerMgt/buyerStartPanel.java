...
public class buyerStartPanel extends javax.swing.JPanel {
    ...
    public buyerStartPanel(JPanel processPanel, Buyer buyer, Business business) {
        ...
        this.refreshOrder();
        refreshCarts();
        txtBalance.setText(String.valueOf(buyer.getBank().getBalance()));
    }
    ...
    //The Order Table, we need to show the orders belong to this buyer
    public void refreshOrder() {
        ...
        //System.out.print("OK");
        //Rewrite the refreshTable function--pre:get from the orderlist in single buyer
        //now: directly populate from the business----WorkQueque
        WorkQueue wq = business.getWq();
        //need to add a filter
        for (WorkRequest wr : wq.getWorkRequestList()) {

            if (wr.getOrder().getBuyer().equals(buyer)) {
                //how to get the wr???
                Order ovo = wr.getOrder();
                Object row[] = new Object[8];
                row[0] = wr;
    ...
    private void initComponents() {
        ...
        btnReceived.setBackground(new java.awt.Color(255, 255, 255));
        btnReceived.setFont(new java.awt.Font("Viner Hand ITC", 1, 18)); // NOI18N
        btnReceived.setForeground(new java.awt.Color(255, 102, 102));
        btnReceived.setText("Comfirm a delivery");
            ...
            .addGroup(myOrderPanelLayout.createSequentialGroup()
                ...
                .addGroup(myOrderPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addGroup(myOrderPanelLayout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addComponent(btnAppeal, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)))
    ...
    private void btnWineDetailActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnWineDetailActionPerformed
        ...
        int rowSelect = tblMyOrders.getSelectedRow();
        if (rowSelect >= 0) {
            WorkRequest wr = (WorkRequest) tblMyOrders.getValueAt(rowSelect, 0);
 ...
 /*Wine w = (Wine) tblBrowseProduct.getValueAt(rowSelect, 0);
        ...
        if (rowSelect >= 0) {
            ...
            //Order ovo = (Order) tblMyOrders.getValueAt(rowSelect, 0);
            Order ovo = wr.getOrder();
            if (ovo.getOrderStatus().equals("Delivered")) {
                ovo.setOrderStatus("Return started");
                wr.setStatus("Pending");
                refreshOrder();
                returnPanel rp = new returnPanel(processPanel, wr.getOrder().getWine());
                processPanel.add("returnPanel", rp);
                CardLayout card = (CardLayout) processPanel.getLayout();
                card.next(processPanel);
            } else {
                JOptionPane.showMessageDialog(null, "You cannot change this order's status");
            }
...
//By defualt, the "status" of the newly added order is "order placed", which can be further updated by the seller.
    private void btnCheckoutActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCheckoutActionPerformed
        // TODO add your handling code here:
        //One-Clicked Checked Out Button
        int selectedRow = tblBrowseProduct.getSelectedRow();
        if (selectedRow < 0) {
            JOptionPane.showMessageDialog(null, "Pls select a row!!", "Warning", JOptionPane.WARNING_MESSAGE);
            return;
        ...
        try {
            if (fetchQty <= wseller.getNumberofInventory()) {
                //Valid Number of Wines
                WorkRequest wr = business.getWq().addWorkQuest();
                Order o1 = new Order();
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                String str = sdf.format(new Date());
                wr.setRequestDate(str);
                wr.setOrder(o1);
                wr.setStatus("Pending");
                        ...
                        for (Wine wx : sls.getWineList()) {
                            ...
                            if (wx.getWineName().equals(wseller.getWineName())) {
                                ...
                                if (fetchQty <= wx.getNumberofInventory()) {
                                    ...
                                    //NOW we dont have OrderList in business any more;
                                    //We need to create a WOrk Request and add the Wine's in the Work Request;
    ...
    private void btnReceivedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReceivedActionPerformed
        ...
        if (rowSelect >= 0) {
            ...
            //Order ovo = (Order) tblMyOrders.getValueAt(rowSelect, 0);
            Order ovo = wr.getOrder();
            if (ovo.getOrderStatus().equals("Shipped")) {
                ovo.setOrderStatus("Delivered");
                wr.setStatus("Pending");
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                String str = sdf.format(new Date());
                wr.setRequestDate(str);
                wr.setResolveDate(null);
                refreshOrder();
                JOptionPane.showMessageDialog(null, "You have recieved the product.");
            } else {
                JOptionPane.showMessageDialog(null, "You cannot change this order's status");
            }
    ...
    private void btnAppealActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAppealActionPerformed
        // TODO add your handling code here:
        // change order status to "Appealed"
        int rowSelect = tblMyOrders.getSelectedRow();
        if (rowSelect >= 0) {
            WorkRequest wr = (WorkRequest) tblMyOrders.getValueAt(rowSelect, 0);
            //Order ovo = (Order) tblMyOrders.getValueAt(rowSelect, 0);
            Order ovo = wr.getOrder();
            ovo.setOrderStatus("Appealed");
            wr.setStatus("Pending");
            refreshOrder();
            JOptionPane.showMessageDialog(null, "This order is appealing");
        } else {
            JOptionPane.showMessageDialog(null, "Please select any row");
        }
